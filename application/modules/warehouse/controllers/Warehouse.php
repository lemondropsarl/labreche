...
class Warehouse extends MX_Controller {
	...
	public function create_entry_in()
	{
		...
		$si_qty = $this->input->post('si_qty');
		$si_date = $this->input->post('si_date');
		$si_user_id = $this->session->userdata('user_id');

		//add the location
		$model = array(
			'prod_loc_prod_id' => $product_id,
			'prod_loc_zone_id' =>$this->input->post('prod_zone_id'),
			'prod_loc_shelf_id' => $this->input->post('prod_shelf_id')
		);
